{if $providers_list}
    {if $auth.user_id}
                ...
                {foreach $providers_list as $provider_data}
                    {if in_array($provider_data.provider, $linked_providers)}
                        <div class="ty-hybrid-auth__icon ty-float-left">
                            ...
                            <img src="{$provider_data.icon|replace:'/flat_24x24/':'/flat_64x64/'|replace:'/flat_32x32/':'/flat_64x64/'}" title="{__("hybrid_auth.linked_provider")}" alt="{if $provider_data.display_name && $provider_data.display_name|strtolower !== $provider_data.provider}{$provider_data.display_name|strtolower}{else}{$provider_data.provider}{/if}"/>
                        </div>
                    {/if}
                {/foreach}
                    ...
                    {if !in_array($provider_data.provider, $linked_providers)}
                        <div class="ty-hybrid-auth__icon ty-float-left">
                            <a class="cm-link-provider ty-link-unlink-provider" data-idp="{$provider_data.provider_id}" data-provider="{$provider_data.provider}">
                                ...
                                <img
                                    src="{$provider_data.icon|replace:'/flat_24x24/':'/flat_64x64/'|replace:'/flat_32x32/':'/flat_64x64/'}"
        ...
        {/hook}
    {else}
        {include file="addons/hybrid_auth/views/auth/components/login_buttons.tpl" is_registration=true}
    {/if}
{/if}
