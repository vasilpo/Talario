{if $order_info}
    ...
    {assign var="booking_info" value=$order_info.booking_info_data|default:array()}
    ...
    <div class="ty-booking-details">
        <section class="ty-booking-details__hero">
            <div class="ty-booking-details__hero-main">
                <h1 class="ty-booking-details__title">{$order_info.booking_product_name}</h1>

                {if $order_info.booking_type_name}
                    <div class="ty-booking-details__type-badge">{$order_info.booking_type_name}</div>
                {/if}
                ...
                <div class="ty-booking-details__meta">
                    {if $booking_info.booking_type == "T" && $booking_info.booking_date}
                        <div class="ty-booking-details__meta-item">
                            ...
                            <span>
                                ...
                                {if $order_info.booking_slot_start}
                                    {__("bookings.booking_time_prefix")} {$order_info.booking_slot_start}
                                {/if}
                            </span>
                        </div>
                    ...
                    {if $order_info.booking_address}
                        <div class="ty-booking-details__meta-item">
                            <i class="ut2-icon-location_on ty-booking-details__meta-icon"></i>
                            <span>{$order_info.booking_address}</span>
                        </div>
                    {/if}
                    ...
                    {if $order_info.notes}
                        <div class="ty-booking-details__meta-item">
                            <i class="ut2-icon-article ty-booking-details__meta-icon"></i>
                                <span>{$order_info.notes}</span>
                        </div>
                    {/if}
                    <p class="ty-muted">{__("bookings.booking_number")}: #{$order_info.order_id}</p>
                </div>
            </div>
        ...
        <div class="ty-booking-details__grid{if !$qr_code_url} ty-booking-details__grid--single{/if}">
            <div class="ty-booking-details__content">
                <section class="ty-booking-details__card">
                    <h2 class="ty-booking-details__card-title">{__("bookings.lesson_information")}</h2>
                        ...
                        <div class="ty-booking-details__lesson-main">
                            <div class="ty-booking-details__lesson-head">
                                <div class="ty-booking-details__lesson-summary">
                                    <div class="ty-booking-details__lesson-name">{$order_info.booking_product_name}</div>
                            ...
                            <div class="ty-booking-details__lesson-facts">
                                {if $order_info.booking_address}
                                    <div class="ty-booking-details__fact">
                                        <div class="ty-booking-details__fact-label">{__("bookings.booking_address")}:</div>
                                        <div class="ty-booking-details__fact-value">{$order_info.booking_address}</div>
                                    </div>
                                {/if}

                                {if $booking_info.booking_type == "T" && $booking_info.booking_date}
                                    <div class="ty-booking-details__fact">
                                        <div class="ty-booking-details__fact-label">{__("bookings.booking_date_time")}:</div>
                                        <div class="ty-booking-details__fact-value">
                                            {$booking_info.booking_date|date_format:"%e %B %Y"}
                                            {if $order_info.booking_slot_start}, {$order_info.booking_slot_start}{/if}
                                        </div>
                                    </div>
                                {elseif $booking_info.from || $booking_info.to}
                                    <div class="ty-booking-details__fact">
                                ...
                                {if $order_info.booking_type_name}
                                    <div class="ty-booking-details__fact">
                                        <div class="ty-booking-details__fact-label">{__("bookings.lesson_format")}:</div>
                                        <div class="ty-booking-details__fact-value">{$order_info.booking_type_name}</div>
                                    </div>
                                {/if}

                                {if $order_info.booking_age}
                                    <div class="ty-booking-details__fact">
                                        <div class="ty-booking-details__fact-label">{__("bookings.booking_age")}:</div>
                                        <div class="ty-booking-details__fact-value">{$order_info.booking_age}</div>
                                    </div>
                                {/if}
                            </div>
                        </div>
                ...
                <section class="ty-booking-details__card">
                    <h2 class="ty-booking-details__card-title">{__("bookings.payment_block_title")}</h2>

                    <div class="ty-booking-details__payment">
                        <div class="ty-booking-details__payment-row">
                            <span>{__("bookings.booking_cost")}:</span>
                            <strong>{include file="common/price.tpl" value=$order_info.booking_products_total}</strong>
                        </div>

                        <div class="ty-booking-details__payment-row">
                            <span>{__("bookings.points_spent")}:</span>
                            <strong class="ty-booking-details__payment-accent">
                                {if $order_info.booking_points_cost|floatval > 0}-{/if}{include file="common/price.tpl" value=$order_info.booking_points_cost}
                            </strong>
                        </div>

                        <div class="ty-booking-details__payment-total">
                            <span><strong>{__("bookings.total_amount")}:</strong></span>
                            <strong>{include file="common/price.tpl" value=$order_info.booking_paid_total}</strong>
                        </div>
                    </div>
                </section>

                <section class="ty-booking-details__card">
                    <h2 class="ty-booking-details__card-title">{__("bookings.actions_title")}</h2>
                    ...
                    {if $addons.vendor_communication.show_on_order === "YesNo::YES"|enum && !$vendor_communication_order_thread}
                        <div class="ty-booking-details__actions">
                            {include
                                ...
                                title=__("bookings.contact_support_to_cancel")
                    ...
                    <div class="ty-booking-details__actions-note">
                        {__("bookings.cancellation_terms_prefix")}
                        <span class="ty-booking-details__actions-note-accent">{__("bookings.cancellation_terms_highlight")}</span>.
                    </div>
                </section>
            </div>
                ...
                <aside class="ty-booking-details__sidebar">
                    <section class="ty-booking-details__qr-card">
                        ...
                        <a class="ty-booking-details__qr-download ty-btn ty-btn__primary" href="{$qr_code_url}" download="booking-qr-{$order_info.order_id}.png">
                            {__("bookings.download_qr_code")}
                        </a>

                        {if $order_info.booking_calendar_event_available}
                            <a class="ty-booking-details__calendar-link ty-btn ty-btn__secondary" href="{"bookings.calendar_event?order_id=`$order_info.order_id`"|fn_url}">
                                <i class="ut2-icon-addchart"></i>
                                {__("bookings.add_to_calendar")}
                            </a>
                        {/if}
                    </section>
                </aside>
