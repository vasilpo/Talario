...
{foreach $item.subitems as $subitem}
    {hook name="ab__fast_navigation:second_level_item"}
             ...
             <span class="ab-fn-image-wrap{if !$subitem.image || !$subitem.image.icon} ab-fn-no-image-wrapper{/if}" style="width: {$first_level_icon_width}px;">
                 {if $subitem.image}
                     {if $subitem.ab__fn_use_origin_image == 'Y'}
                         {include file="common/image.tpl" ab__loading_spinner=true images=$subitem.image}
                     {else}
                         {include file="common/image.tpl" ab__loading_spinner=true image_height=$first_level_icon_width image_width=$first_level_icon_width images=$subitem.image}
                     {/if}
                 {else}
