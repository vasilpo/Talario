{if $page.page_type == $smarty.const.PAGE_TYPE_FORM && $page.page_id == $addons.sd_design_changes.apply_vendor_page_id}
    ...
    {else}
        ...
        <div class="sd-company-apply-page">
            ...
            <div class="sd-company-apply-page__fields" id="apply_for_vendor_account">
                ...
                <form class="sd-fields" action="{""|fn_url}" method="post" name="forms_form" enctype="multipart/form-data">
                        ...
                        {elseif $element.element_type != $smarty.const.FORM_IP_ADDRESS && $element.element_type != $smarty.const.FORM_REFERER}
                            <div class="ty-control-group {if $element.element_type == $smarty.const.FORM_PHONE}cm-mask-phone-group{/if}" {if $element.element_type == $smarty.const.FORM_PHONE}data-ca-phone-mask-group-id="elm_{$element.element_id}"{/if}>
                                ...
                                {elseif $element.element_type == $smarty.const.FORM_INPUT}
                                    <input placeholder=" " id="elm_{$element.element_id}" class="ty-form-builder__input-text ty-input-text {if $element.position == 1}cm-focus {/if}" size="50" type="text" name="form_values[{$element.element_id}]" value="{$form_values.$element_id}" />

                                {elseif $element.element_type == $smarty.const.FORM_TEXTAREA}
                                    <textarea placeholder=" " id="elm_{$element.element_id}" class="ty-form-builder__textarea" name="form_values[{$element.element_id}]" cols="67" rows="10">{$form_values.$element_id}</textarea>
                                ...
                                {elseif $element.element_type == $smarty.const.FORM_EMAIL || $element.element_type == $smarty.const.FORM_NUMBER}
                                    ...
                                    {/if}
                                    <input placeholder=" " id="elm_{$element.element_id}" class="ty-input-text {if $element.position == 1}cm-focus {/if}" size="50" type="text" name="form_values[{$element.element_id}]" value="{$form_values.$element_id}" />
                            ...
                            {hook name="pages:form_elements"}{/hook}
                            </div>
                        {/if}
                    {/foreach}

                    {include "common/image_verification.tpl" option="form_builder"}

                    {hook name="pages:additional_elements"}{/hook}
                    ...
                    <div class="sd-company-apply-page__button">
                        {include "buttons/button.tpl"
                            ...
                            but_name="dispatch[pages.send_form]"
                        }
                    </div>
                </form>
            </div>
