    ...
    <div class="ty-dropdown-box" id="cart_status_{$dropdown_id}">
        ...
        <a href="{"checkout.cart"|fn_url}" class="ac-title ty-hand">
            {hook name="checkout:dropdown_title"}
                {if $cart.amount}
                    <i class="ut2-icon-outline-cart filled">
                        <span class="ty-minicart-count">{$cart.amount}</span>
                    </i>
                {else}
                ...
                <span>{__("cart")}{include_ext file="common/icon.tpl" class="ut2-icon-outline-expand_more"}</span>
            {/hook}
        </a>
        </div>
