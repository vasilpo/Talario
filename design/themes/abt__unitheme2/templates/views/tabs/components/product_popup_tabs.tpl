    ...
    {foreach from=$tabs item="tab" key="tab_id"}
        {if $tab.show_in_popup == "Y" && $tab.status == "A"}
            ...
            {if $smarty.capture.$tab_content_capture|trim}
                <div class="ut2-pb__popup-tabs-item">{include_ext file="common/icon.tpl" class="ty-icon-popup ut2-pb__popup-tabs-icon"}<a id="{$tab.html_id}" class="cm-dialog-opener" data-ca-target-id="content_block_popup_{$tab_id}" rel="nofollow">{$tab.name}</a></div>
