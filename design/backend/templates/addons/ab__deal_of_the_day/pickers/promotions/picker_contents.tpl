...
var id = $(this).val();
promotions[id] = $('#promotion_title_' + id).text().trim();
});
{literal}
...
<td>
<label for="checkbox_id_{$promotion.promotion_id}" id="promotion_title_{$promotion.promotion_id}">
{$promotion.name}
{if $promotion.company_id && fn_allowed_for("MULTIVENDOR")}
<p class="muted"><small>{fn_get_company_name($promotion.company_id)}</small></p>
{/if}
</label>
</td>
