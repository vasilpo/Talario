...
<th width="15%"><a class="cm-ajax" href="{"`$c_url`&sort_by=name&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id="pagination_contents_motivation_items">{__("name")}{if $search.sort_by == "name"}{$c_icon nofilter}{else}{$c_dummy nofilter}{/if}</a></th>
<th width="25%" class="mobile-hide">{__("categories")}</th>
<th width="25%" class="mobile-hide">{__("ab__mb.destinations")}</th>
<th width="15%" class="mobile-hide">{__("products")}</th>
...
{if $motivation_item.{"exclude_`$_tmp_obj`"} == "YesNo::NO"|enum}
{__('display_on')}:&nbsp;
{else}
...
<td class="mobile-hide" data-th="{__("products")}">
{$_tmp_obj = 'products'}
<div class="row-status">
{if $motivation_item.{"`$_tmp_obj`_ids"}}
{if $motivation_item.{"exclude_`$_tmp_obj`"} == "YesNo::NO"|enum}
