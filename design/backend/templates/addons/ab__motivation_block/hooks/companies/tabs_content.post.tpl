{if "MULTIVENDOR"|fn_allowed_for && (fn_check_view_permissions('ab__motivation_block.view', 'GET') || $addons.vendor_privileges.status != "ObjectStatuses::ACTIVE"|enum)}
<div id="content_ab__motivation_block" class="hidden">
{if ($auth.user_type === 'UserTypes::VENDOR'|enum) && $addons.ab__motivation_block.description_type == 'smarty'}
{$addons.ab__motivation_block.description_type = 'html'}
{/if}
{if $ab__mb_items}
{foreach from=$ab__mb_items item="ab__mb_item" key="_key"}
{hook name="ab__mb:companies_motivation_item"}
{if $ab__mb_item.status != "ObjectStatuses::DISABLED"|enum}
<div style="margin: 20px 0">
<h5>{$ab__mb_item.name}</h5>
<p class="muted">{__("storefront")}: {$app["storefront.repository"]->findById($ab__mb_item.storefront_id)->name}</p>
{if $ab__mb_item.template_path == 'addons/ab__motivation_block/blocks/components/item_templates/custom_content.tpl'}
...
input.prop('checked', (data.value === {"YesNo::YES"|enum}));
} else {
