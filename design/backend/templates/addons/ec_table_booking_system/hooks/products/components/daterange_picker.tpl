{if $enable_data_event && !$data_event}
    {$data_event = "ec_datepicker_range_selector"}
{/if}
...
<div id="{$id}"
    ...
    data-ca-show-ranges="{$show_ranges|default:"false"}"
    {if $data_event}data-ca-event="{$data_event}"{/if}>
    <input type="hidden" id="{$id}_start_date_value" class="start_date_value" name="{$start_date_value_name|default:"start_date"}" value="{$start_date}" />
    <input type="hidden" id="{$id}_end_date_value" class="end_date_value"  name="{$end_date_value_name|default:"end_date"}" value="{$end_date}" />
...
{if $enable_data_event}
    <script>
        $.ceEvent('on', '{$data_event}', function (el, selected_from, selected_to, start, end) {
            console.log(el);
            $(el).children('.start_date_value').val(selected_from);
             $(el).children('.end_date_value').val(selected_to);
        });
    </script>
{/if}