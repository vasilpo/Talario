...
<div class="sidebar-field">
    <label for="names">{__("ec_table_booking_system.customer_name")}</label>
    <input type="text" name="name" id="names" value="{$search.name}" size="30" />
</div>
<div class="sidebar-field">
    <label for="emails">{__("email")}</label>
    <input type="text" name="email" id="emails" value="{$search.email}" size="30" />
</div>
<div class="sidebar-field">
    <label for="status">{__("status")}</label>
  <div class="controls">
        <select name="status" id="booked_status">
            <option value="">--</option>
            <option value="A" {if $search.status == "A"}selected="selected"{/if}>{__("ec_table_booking_system.booked")}</option>
            <option value="D" {if $search.status == "D"}selected="selected"{/if}>{__("ec_table_booking_system.cancelled")}</option>
        </select>
        </div>
</div>
{/capture}
...
<div class="control-group">
    ...
    <div class="controls checkbox-list">
        {include file="common/status.tpl" status=$search.status display="checkboxes" name="order_statuses" columns=5}
    </div>
</div>
{/if}
</div>
