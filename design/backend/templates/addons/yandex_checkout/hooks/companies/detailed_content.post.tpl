{if $is_yandex_checkout_for_marketplaces_used && !$runtime.company_id}
    ...
    <div class="control-group">
        ...
        <div class="controls">
            <p class="muted description">
                {__("yandex_checkout.yandex_checkout_for_marketplaces_vendor_info") nofilter}
            </p>
        </div>
    </div>
    <div class="control-group">
    ...
    <div class="control-group">
        <label class="control-label" for="yandex_tax_code_{$runtime.company_id}">{__("rus_taxes.tax_system")}:</label>
        <div class="controls">
            <select name="company_data[yandex_tax_code]" id="yandex_tax_code_{$runtime.company_id}">
                <option value="">{__("none")}</option>
                {foreach $yandex_tax_codes as $code => $tax_code_text}
                    <option value="{$code}"{if $company_data.yandex_tax_code == $code} selected{/if}>{$tax_code_text}</option>
                {/foreach}
            </select>
        </div>
    </div>
    <div class="control-group">
        ...
        <div class="controls">
            <p class="muted description">
                ...
                {else}
                    {__("yandex_checkout.marketplace_fee_disabled_notice", ["[href]" => $addons_page]) nofilter}
                {/if}
            </p>
        </div>
    </div>
