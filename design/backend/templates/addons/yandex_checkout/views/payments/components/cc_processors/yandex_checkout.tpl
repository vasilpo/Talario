...
<div id="yandex_checkout_payment_instruction_{$payment_id}" class="in collapse">
    ...
    {if fn_get_storefront_protocol() != "https"}
        {__("yandex_checkout.server_https") nofilter}
    {/if}
</div>
<input type="hidden"
...
<div id="yandex_checkout_payment_settings_{$payment_id}" class="in collapse">
    ...
    <div class="control-group">
        <label class="control-label" for="yandex_checkout_tax_code_{$payment_id}">{__("rus_taxes.tax_system")}:</label>
        <div class="controls">
            <select name="payment_data[processor_params][yandex_tax_code]" id="yandex_checkout_tax_code_{$payment_id}">
                <option value="">{__("none")}</option>
                {foreach $yandex_tax_codes as $code => $tax_code_text}
                    <option value="{$code}"{if $processor_params.yandex_tax_code == $code} selected{/if}>{$tax_code_text}</option>
                {/foreach}
            </select>
        </div>
    </div>
