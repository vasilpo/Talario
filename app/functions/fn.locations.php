...
function fn_get_available_destination($location)
{
    ...
    if (!empty($country)) {
        ...
        if (empty($destinations)) {
            ...
            $__dests = db_get_array(
                'SELECT a.* FROM ?:destination_elements as a LEFT JOIN ?:destinations as b ON b.destination_id = a.destination_id WHERE b.status = ?s ?p',
                ObjectStatuses::ACTIVE,
                $condition
            );

            foreach ($__dests as $v) {
...
function fn_get_destinations_for_picker(array $params, $lang_code = DESCR_SL)
{
    ...
    foreach ($shipping_info['rates'] as $key_rate => $rate) {
        if (!isset($rate['rate_value']) || !is_array($rate['rate_value'])) {
            continue;
        }
