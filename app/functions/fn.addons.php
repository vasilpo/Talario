...
/**
 * Gets actions schema declared into specified add-on.
 *
 * @param string $addon Name of specified addon.
 *
 * @return array<string, array<string|null|array<string>>>
 */
function fn_get_actions_schema_for_addon($addon)
{
    $all_formatted_actions = fn_format_actions_to_menu_items(
        fn_get_schema('menu', 'actions', 'php', false, [])
    );

    $addon_formatted_actions = fn_format_actions_to_menu_items(
        fn_get_schema('menu', 'actions', 'php', false, [$addon])
    );

    return array_diff_key($addon_formatted_actions, $all_formatted_actions);
}

/**
 * Gets actions schema declared into specified add-on.
 *
 * @param array $actions_schema Actions schema.
 *
 * @return array
 *
 ...
 */
function fn_format_actions_to_menu_items(array $actions_schema)
{
    if (empty($actions_schema)) {
        return [];
    }

    $formatted_actions = [];
    foreach (array_values($actions_schema) as $action) {
        foreach ($action as $action_id => $action_params) {
            if (empty($action_params['href'])) {
                continue;
            }
            $formatted_actions[$action_params['href']] = [
                'id' => $action_id,
                'title' => $action_params['text'] ?? null,
                'parent' => null
            ];
        }
    }
        ...
        if (in_array($params['type'], ['any', 'installed', 'active', 'disabled'])) {
            ...
            if (!empty($installed_addons[$addon])) {
                ...
                if ($addons[$addon]['status'] === ObjectStatuses::ACTIVE) {
                    $action_items = fn_get_actions_schema_for_addon($addon) ?? [];
                    $menu_items = fn_get_menu_schema_for_addon($addon) ?? [];
                    if (!empty($menu_items = array_merge($action_items, $menu_items))) {
