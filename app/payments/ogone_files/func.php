...
function fn_ogone_subtrim($string, $length = null, $start = 0)
{
    $string = trim($string);
...
function fn_ogone_get_status($code)
{
    $code = (string) $code;
...
function fn_ogone_get_status_description($code)
{
    $code = (string) $code;
    ...
    if (isset($descriptions[$code])) {
        return array(true, $descriptions[$code]);
    }

    return array(false, 'Status code: ' . $code);
}

function fn_ogone_get_error_description($code)
{
    $code = (string) $code;
    ...
    return array(false, 'Error code: ' . $code);
}

function fn_ogone_calculate_signature($params_list, $secret)
{
    $request_to_sign = array();

    foreach ($params_list as $param => $value) {
        ...
        $request_to_sign[] = $param . '=' . $value;
    }

    sort($request_to_sign);
    $request_to_sign[] = '';

    $string_to_sign = implode($secret, $request_to_sign);

    $signed_string = sha1($string_to_sign);
