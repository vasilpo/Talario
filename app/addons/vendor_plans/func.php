...
/**
 * Fill content information for block with vendor plan limits on dashboard
 *
 ...
 *  }
 */
function fn_get_vendor_plans_dashboard_block_data()
{
    $content_data = null;
    if ($company = Company::current()) {
        ...
        $content_data = [
            'title_button' => [
                ...
                'href' => 'companies.update?company_id=' . $company->company_id . '&selected_section=plan'
            ],
            'number' => $usage['products']['percentage'] . '%',
            'bar_chart' => [
                'id' => 'vendor_plans_analytics_card_vendor_plan_bar_chart',
                'title' => __('vendor_plans.dashboard.analytics_card.vendor_plan_limits', [
                    '[name]' => $company->plan['plan']
                ]),
                'content' => [
                    [
                        ...
                        'type' => 'primary',
                        'value' => (float) $usage['products']['current'],
                        'total' => $usage['products']['limit'],
                        'ratio' => (float) $usage['products']['percentage']
                    ],
                    [
                        'id' => 'vendor_plans_analytics_card_vendor_plan_bar_chart_revenue',
                        'name' => __('vendor_plans.revenue'),
                        'href' => 'companies.balance',
                        'is_price' => true,
                        'value' => (float) $usage['revenue']['current'],
                        'total' => $usage['revenue']['limit'],
                        'ratio' => (float) $usage['revenue']['percentage']
                    ]
                ]
            ]
        ];
        ...
        if (!$is_vendor_admin_creation_needed) {
            ...
            $event_dispatcher->dispatch(
                ...
                [
                    ...
                    'new_account' => $action === 'add' ? true : false,
                ]
            );
        }
...
/**
 * Skip products with unavailable categories.
 *
 ...
 * @param array                 $object                    The whole record data
 *
 * @return void
 *
...
function fn_vendor_plans_import_skip_products_with_unavailable_categories(
    ...
    foreach ($categories_paths as $category_path) {
        ...
        if (!empty($paths)) {
            foreach ($paths as $name) {
                $sql = 'SELECT ?:categories.category_id FROM ?:category_descriptions'
                    ...
                    . ' LEFT JOIN ?:storefronts_companies ON ?:categories.storefront_id = ?:storefronts_companies.storefront_id'
                    . ' WHERE ?:category_descriptions.category = ?s AND lang_code = ?s AND parent_id IN (?n)'
                    . ' AND (?:storefronts_companies.company_id IS NULL OR ?:storefronts_companies.company_id = ?s)';

                $category_ids = db_get_fields($sql, $name, $lang_code, $parent_ids, $company_id);
        ...
        if (empty(array_intersect($category_ids, $company_category_ids))) {
        ...
        foreach ($category_ids as $_category_id) {
            if (in_array($_category_id, $company_category_ids)) {
                continue;
            }

            $forbidden_category_ids[] = $_category_id;
        }

        if (empty($forbidden_category_ids)) {
            return;
        }
        $object['forbidden_category_ids'] = fn_get_category_ids_with_parent($forbidden_category_ids);
    }
}
