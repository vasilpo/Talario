...
function fn_rus_dellin_install()
{
    ...
    fn_cities_read_cities_by_chunk($path, CITIES_FILE_READ_CHUNK_SIZE, 'fn_rus_dellin_add_cities_in_table');
...
function fn_rus_dellin_get_code_city($params, $lang_code = CART_LANGUAGE)
{
    ...
    $code_kladr = db_get_field(
        'SELECT code_kladr FROM ?:city_descriptions'
        . ' LEFT JOIN ?:cities'
        . ' ON ?:city_descriptions.city_id = ?:cities.city_id'
        . ' LEFT JOIN ?:rus_dellin_cities_link'
        . ' ON ?:cities.city_id = ?:rus_dellin_cities_link.city_id'
 ...
 *
 * Actions performed:
 * - Generates the correct keys of "shippings_extra" and "arrival_terminal" in suborder cart.
 *
 ...
 * @return void
 */
function fn_rus_dellin_place_suborders_pre($order_id, $cart, $auth, $action, $issuer_id, &$suborder_cart, $key_group, $group)
{
    if (
    ...
    if (!isset($suborder_cart['arrival_terminal'][$key_group])) {
        return;
    }

    $suborder_cart['arrival_terminal'] = [$suborder_cart['arrival_terminal'][$key_group]];
}
