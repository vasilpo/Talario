...
{
    /** @const int Used to limit the variations displayed in the pop-up to create variations */
    const COMBINATIONS_COUNT_ALLOWED_FOR_DISPLAY = 100;
     ...
     *
     * @param bool $is_apply_limit Whether variations count limit to display is used
     *
     * @return array<string, array<string, mixed>>
     */
    public function getCombinations(bool $is_apply_limit = false)
    {
        ...
        if (
            $is_apply_limit
            && $this->checkCombinationVariationsCountReachedLimit()
        ) {
            return [];
        }
    ...
    /**
     * Checks whether the combination variations count has reached the limit
     *
     * @return bool
     */
    protected function checkCombinationVariationsCountReachedLimit()
    {
        $combination_count = $this->getCombinationVariationsCount();

        if ($combination_count > GenerateVariationsForm::COMBINATIONS_COUNT_ALLOWED_FOR_DISPLAY) {
            return true;
        }
    ...
    /**
     * Gets combination variations count
     *
     * @return int
     */
    protected function getCombinationVariationsCount()
    {
        $features_variants_map = $this->getFeaturesVariantsMap();

        if (empty($features_variants_map)) {
            return 0;
        }

        $combination_count = 1;
        foreach ($features_variants_map as $variants) {
            $combination_count *= count($variants);
        }
