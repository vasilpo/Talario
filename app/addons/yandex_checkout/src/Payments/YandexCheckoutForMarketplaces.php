...
class YandexCheckoutForMarketplaces
{
    ...
    public function __construct(
        ...
        $this->client->setLogger($this->logger->getApiClientLoggerWrapper());
    ...
    public function createPayment(array $order_info, array $processor_data)
    {
        ...
        $description = __('order_id') . ': ' . $order_info['order_id'];
        $params = [
            'amount'       => [
        ...
        $response = $this->client->createPayment($params);
        if (!$response) {
            return new CreatePaymentResponse([]);
        }
        return $response;
    ...
    /**
     * @param array<string, string|float|int> $order_info      Order info
     * @param int                             $shop_id         Shop ID
     * @param string                          $payment_mode    Payment mode
     * @param string                          $settlement_type Settlement type
     ...
     */
    public function createReceipt(array $order_info, $shop_id, $payment_mode, $settlement_type, $tax_system_code = null)
    {
        if (!$shop_id) {
