...
class ChangeOrderStatusWithYandexCheckoutForMarketplaces
{
    ...
    protected function getPaymentInfo(YandexCheckoutForMarketplaces $payment_processor)
    {
        ...
        try {
            ...
            if ($payment_info === null || $payment_info->getStatus() !== PaymentStatus::SUCCEEDED) {
                return null;
            }
    ...
    protected function sendReceipt(YandexCheckoutForMarketplaces $payment_processor, $payment_mode)
    {
        ...
        try {
            ...
            if (
                !empty($company_data['agent_type'])
                && AgentType::valueExists($company_data['agent_type'])
                && !empty($company_data['tax_number'])
            ) {
                $order_info['yandex_checkout_agent_type'] = $company_data['agent_type'];
                $order_info['yandex_checkout_supplier'] = [
                    'name' => $company_data['company'],
                    'inn'  => $company_data['tax_number'],
                ];
            }
            ...
            $receipt = $payment_processor->createReceipt(
                ...
                $company_data['yandex_tax_code'] ?: null
            );
