...
class ReceiptService
{
    ...
    public function getReceiptFromOrder(array $order_info, $payment_mode)
    {
        ...
        $receipt = ['customer' => $customer, 'items' => $products];
        if (!empty($order_info['payment_method']['processor_params']['yandex_tax_code'])) {
            $receipt['tax_system_code'] = (int) $order_info['payment_method']['processor_params']['yandex_tax_code'];
        }
     ...
     * @param int|null                        $tax_system_code Tax system code
     *
     * @return array{
     *    type: string,
     *    payment_id: string,
     ...
     * }
     */
    public function getPaymentReceiptFromOrder(
        array $order_info,
        $shop_id = null,
        $payment_mode = PaymentMode::FULL_PAYMENT,
        $settlement_type = SettlementType::PREPAYMENT,
        ...
        } elseif (!empty($order_info['payment_method']['processor_params']['yandex_tax_code'])) {
            $receipt['tax_system_code'] = (int) $order_info['payment_method']['processor_params']['yandex_tax_code'];
        }
     ...
     *
     * @param array<string, string|array>          $order_info   Original order information.
     ...
     * items: list<array{amount: array{currency: string, value: float}, description: string, quantity: float, vat_code: string, payment_mode: string, payment_subject: string}>
     * }|null
     */
    public function getReceiptFromRefund(array $order_info, array $return_data, $currency, $payment_mode = PaymentMode::FULL_PAYMENT)
    {
