...
function fn_rus_exim_1c_update_product_feature_variant_before_select(
    ...
    $conditions['external_id'] = db_quote('fv.external_id = ?s', $variant['external_id']);
    unset($conditions['variant']);
}

/**
 * Cleans up old import directories
 *
 * @param string $root_path   Full path to the directory storage location
 * @param string $working_dir Working directory name
 *
 * @return void
 */
function fn_rus_exim_1c_cleanup_files_dir($root_path, $working_dir)
{
    if (!is_dir($root_path . $working_dir)) {
        return;
    }

    $max_files_dirs = abs(Registry::ifGet('config.rus_exim_1c.max_dirs_count', 1));
    $dirs = array_diff(
        scandir($root_path),
        ['.', '..'] // remove from array
    );
    ...
    /**
     * Attempts to parse the directory name as a date (timestamp).
     * If it fails to parse, such a directory will be the last in the list.
     */
    usort($dirs, static function ($a, $b) {
        $date_format = RusEximCommerceml::$dir_name_date_format;

        $first = date_create_from_format($date_format, str_replace('1C_', '', $a));
        if ($first) {
            $first = $first->getTimestamp();
        }

        $second = date_create_from_format($date_format, str_replace('1C_', '', $b));
        if ($second) {
    ...
    $count = 0;
    foreach ($dirs as $dir) {
        if ($dir === $working_dir || $count < $max_files_dirs) {
            $count++;
            continue;
        }
