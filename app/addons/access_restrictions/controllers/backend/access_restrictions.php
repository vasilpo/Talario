...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($mode == 'make_permanent') {
        if ($_REQUEST['selected_section'] == 'ip' || $_REQUEST['selected_section'] == 'admin_panel') {
            ...
            $items_data = $_REQUEST['items_data'];
            foreach ($_REQUEST['item_ids'] as $v) {
    ...
    if (
        ...
        if (defined('AJAX_REQUEST')) {
            $redirect_url = fn_url('access_restrictions.manage');
...
if ($mode == 'manage') {
    ...
    Tygh::$app['view']->assign('host_ip', fn_ip_to_db($ip['host']));
}
...
function fn_update_access_restriction_rule($rule_data, $rule_id = 0, $lang_code = DESCR_SL)
{
    if ($rule_data['section'] === 'ip' || $rule_data['section'] === 'admin_panel') {
        $visitor_ip = fn_get_ip();
        if (!empty($visitor_ip)) {
            $visitor_ip['host'] = fn_ip_to_db($visitor_ip['host']);
            $visitor_ip['proxy'] = fn_ip_to_db($visitor_ip['proxy']);
        }
        ...
        if (!empty($rule_data['range_from']) || !empty($rule_data['range_to'])) {
            ...
            if (fn_validate_ip($range_from, true) && fn_validate_ip($range_to, true)) {
                ...
                $_data = [
                    'ip_from' => fn_ip_to_db($range_from),
                    'ip_to' => fn_ip_to_db($range_to),
                    'timestamp' => TIME,
                ...
                if (
                    $rule_data['section'] === 'admin_panel'
                    && Registry::get('addons.access_restrictions.admin_reverse_ip_access') !== YesNo::YES
                    && ($self_block_host || $self_block_proxy)
                ) {
                    $ip_from = !empty($_data['ip_from']) ? (string) fn_ip_from_db($_data['ip_from']) : '';
                    $ip_to = !empty($_data['ip_to']) ? (string) fn_ip_from_db($_data['ip_to']) : '';
                    $proxy = !empty($visitor_ip['proxy']) ? (string) fn_ip_from_db($visitor_ip['proxy']) : '';
                    $host = !empty($visitor_ip['host']) ? (string) fn_ip_from_db($visitor_ip['host']) : '';

                    fn_set_notification('W', __('warning', '', $lang_code), __('warning_of_ip_adding', [
                        '[entered_ip]' => $ip_from . ($_data['ip_from'] === $_data['ip_to'] ? '' : '-' . $ip_to),
                        '[your_ip]' => $self_block_host ? $host : $proxy
                    ], $lang_code));
                } else {
