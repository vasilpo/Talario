...
class Vkontakte extends OAuth2
{
    ...
    protected function exchangeCodeForAccessToken($code)
    {
        ...
        $stored_state = $this->getStoredData('authorization_state');

        // Hybridauth validates state in authenticateFinish() before calling this method.
        // Keep an explicit check here as a second boundary because VK ID requires state
        // to be forwarded to the token endpoint.
        if (
            ...
            throw new InvalidAuthorizationStateException(
                'Provider returned an invalid authorization state.'
            );
        }
    ...
    protected function validateAccessTokenExchange($response)
    {
        ...
        if (!$collection->exists('access_token')) {
            // Never include the raw provider response here: it can contain
            // access/refresh tokens, id_token, email, phone or other PII.
