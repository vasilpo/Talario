...
function fn_delete_seo_name($object_id, $object_type, $dispatch = '', $company_id = null)
{
    ...
    if (!empty($seo_vars['picker'])) {
        $company_condition = $company_id
            ? fn_get_seo_company_condition('?:seo_redirects.company_id', $object_type, $company_id)
            : '';
        db_query(
            'DELETE FROM ?:seo_redirects WHERE object_id = ?i AND type = ?s ?p',
            $object_id,
            $object_type,
            $company_condition
        );
...
{
    $_object_name = str_replace('.', '-', trim($_object_name, '.'));
    $_object_name = preg_replace('/-+/', '-', $_object_name);
...
function fn_get_corrected_seo_lang_code($lang_code, array $seo_settings = [])
{
    if (!empty($seo_settings)) {
        ...
        $default_lang_code = Registry::ifGet('settings.Appearance.frontend_default_language', $lang_code);
...
function fn_get_seo_vars($type = '', $param = '')
{
    ...
    if (!empty($type) && isset($schema[$type])) {
        $res = $schema[$type];
    } elseif (empty($type)) {
        $res = $schema;
    } else {
...
function fn_seo_get_route(array &$req, array &$result, &$area, &$is_allowed_url)
{
    if (
        ...
        && !empty($req['sl'])
        && !in_array($req['sl'], array_keys(Languages::getSimpleLanguages()))
    ) {
        unset($req['sl']);
    ...
    if (!empty($uri)) {
        $lower_case_uri = fn_strtolower($uri);
        if ($lower_case_uri !== $uri) {
            $lower_case_uri = empty($req)
                ? $lower_case_uri
                : $lower_case_uri . '?' . http_build_query($req);
            $result = [INIT_STATUS_REDIRECT, $lower_case_uri, false, true];
        }
        ...
        foreach ($rewrite_rules as $pattern => $query) {
            ...
            if (!empty($objects) && !empty($objects['object_name'])) {
                ...
                if (!empty($_seo)) {
                    ...
                    if ($_seo_valid) {
                        ...
                        if (
                            $_seo['lang_code'] !== fn_get_corrected_seo_lang_code($frontend_default_language)
                            && $use_single_seo_name
                        ) {
                            fn_define('CART_LANGUAGE', !empty($objects['sl']) ? $objects['sl'] : $_seo['lang_code']);
                            fn_redirect($url_query, false, true);
                            return;
                        }
            ...
            if (
                ...
                $req['sl'] = $requested_language;
                $query_string = http_build_query($req);
                $result = [
                    INIT_STATUS_REDIRECT,
                    fn_url($url_query . '?' . $query_string, 'C', 'rel', $requested_language),
                    false,
                    true,
                ];
    ...
    if ($area == 'C') {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET' && empty($_SERVER['X-SEO-REWRITE']) && !empty($req['dispatch'])) {
            ...
            if (strpos($seo_url, 'dispatch=') === false) {
                return [INIT_STATUS_REDIRECT, $seo_url, false, true];
            }
        }
    }
...
function fn_seo_dispatch_before_display()
{
    ...
    if (isset($schema[$controller][$mode])) {
        ...
        if ($is_valid) {
            ...
            if (
                !empty($_REQUEST[$lang_parameter])
                && $_REQUEST[$lang_parameter] !== $default_language
                && Registry::ifGet('addons.seo.seo_language', YesNo::YES) === YesNo::NO
            ) {
