...
class OrderFormator
{
    ...
    public function form(XMLWriter $xml, array $order_data)
    {
        ...
        $order_xml = array_merge($order_xml, $this->formProductsData($order_data, $discount_rate, $this->getRoundedUpPrice((float) $order_data['subtotal_discount'])));
            ...
            [
                SimpleXmlElement::findAlias('discounts') => [
                    SimpleXmlElement::findAlias('discount') => [
                        ...
                        SimpleXmlElement::findAlias('total') => $this->getRoundedUpPrice((float) $order_data['subtotal_discount']),
     ...
     */
    private function formProductsData(array $order_data, $discount_rate, $discount_amount = 0)
    {
        $products_xml = [];
        ...
        $count = count((array) $order_data['products']);
        $counter = 0;
        foreach ((array) $order_data['products'] as $product) {
            $counter++;
            $is_last = $counter === $count;

            $products_xml[][SimpleXmlElement::findAlias('product')] = $this->formProduct($product, $taxes_data, $discount_rate, $discount_amount, $is_last);
        }
     ...
     * @param bool                                                              $is_last         Is last product
     *
     * @return array<string, float|int|string|array>
     */
    private function formProduct(array $product, array $taxes_data, $discount_rate, &$discount_amount = null, $is_last = false)
    {
        $discounts_xml = [];
        $product_discount = 0;
        ...
        if (empty($external_product_id)) {
            ...
            if (count($external_product_id) > 1) {
                foreach ($external_product_id as $_external_product_id) {
                    if (strpos($_external_product_id, '#') !== false) {
                        $external_product_id = [$_external_product_id];
                        break;
                    }
                }
            }
            $external_product_id = array_shift($external_product_id);
        }
                ...
                $discounts_xml = [
                    SimpleXmlElement::findAlias('discounts') => [
                        SimpleXmlElement::findAlias('discount') => [
                            ...
                            SimpleXmlElement::findAlias('total') => $is_last ? $this->getRoundedUpPrice($discount_amount) : $this->getRoundedUpPrice($product_discount),
                            SimpleXmlElement::findAlias('in_total') => 'false',
                        ]
                    ]
                ];

                $discount_amount -= $this->getRoundedUpPrice($product_discount);
            }
        }
