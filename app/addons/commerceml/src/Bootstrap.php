...
use Tygh\Addons\CommerceML\HookHandlers\ProductHookHandler;
...
class Bootstrap implements BootstrapInterface, HookHandlerProviderInterface
{
    ...
    public function getHookHandlerMap()
    {
        return [
            ...
            'get_profile_fields' => static function ($location, $select, &$condition, $params) {
                if (empty($params['cml_export_to_1c'])) {
                    return;
                }
                $condition .= db_quote(' AND ?:profile_fields.cml_export_to_1c = ?s', YesNo::YES);
            },
            'get_feedback_data' => [
