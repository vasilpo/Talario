...
class Logger
{
    ...
    {
        if ($category) {
            $line = sprintf('%s [%s] [%s]: %s', date('Y-m-d H:i:s'), $level, (string) $category, $message);
        } else {
     ...
     *
     * @param \Tygh\Common\OperationResult $result Result intance
     */
    public function logResult(OperationResult $result)
    {
        foreach ($result->getMessages() as $code => $message) {
            $this->info($message, $code);
        }

        foreach ($result->getWarnings() as $code => $message) {
            $this->warning($message, $code);
        }
