...
class ProductImporter
{
    ...
    private function importProductImages(ProductDto $product, ImportStorage $import_storage, array $current_product_data)
    {
        ...
        foreach ($product->images as $image) {
            $filepath_imported = sprintf('%s/%s', $files_dir, ltrim($image->path, '/'));
            $filepath_contented = sprintf('%s/import_files/%s', $files_dir, fn_basename($image->path));

            if (file_exists($filepath_imported)) {
                $filepath = $filepath_imported;
            } elseif (file_exists($filepath_contented)) {
                $filepath = $filepath_contented;
            } else {
                continue;
            }
    ...
    private function fillProductName(ProductDto $product, ImportStorage $import_storage, array $product_data, $lang_code = null)
    {
        ...
        if ($product->is_variation) {
            $product_name = $this->resetProductVariationName($product_name, $product, $import_storage);
        }
     ...
     * @return ?string
     */
    private function resetProductVariationName(?string $product_name, ProductDto $product, ImportStorage $import_storage): ?string
    {
        if (!$product->is_variation) {
            return $product_name;
        }

        $use_product_variation_name_from = $import_storage->getSetting('catalog_importer.use_product_variation_name_from', 'automatically');
        if ($use_product_variation_name_from !== 'automatically' && $use_product_variation_name_from !== 'import') {
            return $product_name;
        }

        if ($use_product_variation_name_from === 'import') {
            $product_name = null;
        } elseif (!empty($product->variation_feature_values)) {
            ...
            foreach ($product->variation_feature_values as $feature_value) {
                /** @var \Tygh\Addons\CommerceML\Dto\ProductFeatureDto $feature_data */
                $feature_data = $import_storage->findEntity(ProductFeatureDto::REPRESENT_ENTITY_TYPE, $feature_value->feature_id->external_id);
                if (!$feature_data) {
                    $all_features_found = false;
                    break;
                }
                $feature_id = $import_storage->findEntityLocalId(ProductFeatureDto::REPRESENT_ENTITY_TYPE, $feature_data->id)->asInt();
                if (!$feature_id) {
                    $all_features_found = false;
                    break;
                }
            ...
            if ($all_features_found && $feature_ids) {
                $feature_purposes = fn_get_product_feature_purposes_by_feature_ids($feature_ids);
                $not_variation_as_one_product_feature_purposes = array_filter($feature_purposes, static function ($purpose) {
                    return  $purpose !== FeaturePurposes::CREATE_VARIATION_OF_CATALOG_ITEM;
                });

                if (empty($not_variation_as_one_product_feature_purposes)) {
                    $product_name = null;
                }
            }
        }
    ...
    private function fillProductFeatures(ProductDto $product, ImportStorage $import_storage, array $product_data, array $current_product_data)
    {
        ...
        foreach ($product->product_feature_values as $product_feature_value) {
            ...
            $feature_type = $this->product_storage->findProductFeatureType($feature_id);
            $is_multiple_values = $feature_type === ProductFeatures::MULTIPLE_CHECKBOX;
            $is_variantless_feature = !in_array($feature_type, ProductFeatures::getSelectableList());
            $existent_variants = !$is_multiple_values
                ? $this->product_storage->findProductFeatureVariants($feature_id)
                : [];

            $value = [];

            foreach ($selected_feature_values as $external_id => $selected_feature_value) {
                if ($selected_feature_value->value_id instanceof IdDto && !$selected_feature_value->value_id->hasLocalId()) {
                    continue;
                }

                if (!$is_variantless_feature) {
                    $default_action = in_array($selected_feature_value->value, $existent_variants)
                        ? LocalIdDto::VALUE_CREATE
                        : $import_storage->getSetting('mapping.feature.default_variant', LocalIdDto::VALUE_CREATE);

                    $selected_feature_value_dto = IdDto::createByExternalId((string) $external_id);
                    $variant_id = $import_storage->findEntityLocalId(
                        ProductFeatureVariantDto::REPRESENT_ENTITY_TYPE,
                        $selected_feature_value_dto,
                        $default_action
                    );
