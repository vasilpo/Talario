...
class OrderImporter
{
    ...
    private function updateOrderNew(OrderDto $order, ImportStorage $import_storage, array $current_order_data, $notify_user)
    {
        ...
        $order_data = array_merge($order->properties->getValueMap(), [
            ...
            'subtotal'          => $order->subtotal === null ? $current_order_data['subtotal'] : $order->subtotal,
            'subtotal_discount' => $order->subtotal_discount === null ? $current_order_data['subtotal_discount'] : $order->subtotal_discount,
            'shipping_cost'     => $order->shipping_cost === null ? $current_order_data['shipping_cost'] : $order->shipping_cost,
            'total'             => 0,
            'discount'          => 0,
            'payment_surcharge' => 0,
        ...
        /** @psalm-suppress ArgumentTypeCoercion */
        $order_data = $this->fillOrderTotals($order_data);
     ...
     *  total: int,
     *  discount: int,
     *  payment_surcharge: float|int,
     *  stored_discount: string,
     *  products: array<array-key, int|float|string>|array<empty, empty>
     * } $order_data
     */
    private function fillOrderTotals(array $order_data)
    {
