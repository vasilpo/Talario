...
class OrderStorage
{
    ...
    public function getOrderInfo($order_id)
    {
        ...
        $order_info['cml_profile_fields'] = [];

        $profile_fields = $this->getCustomProfileFields();

        // Gets profile fields for order
        foreach ($order_info['fields'] as $field_id => $field_value) {
            if (empty($field_value) || empty($profile_fields[$field_id])) {
                continue;
            }

            $field_description = $profile_fields[$field_id];

            if (is_numeric($field_description)) {
                continue;
            }

            $order_info['cml_profile_fields'][$field_description] = $field_value;
        }
    ...
    {
        if (empty($this->custom_profile_fields)) {
            $profile_fields = fn_get_profile_fields('ALL', [], CART_LANGUAGE, [
                'get_custom'       => true,
                'cml_export_to_1c' => true
            ]);
            ...
            foreach ($profile_fields as $section) {
                foreach ($section as $field_id => $field) {
                    ...
                    $this->custom_profile_fields[(int) $field_id] = $field['description'];
                }
            }
        }
    ...
    public function updateOrder(array $order_data, $order_id, $notify_user, $error_message)
    {
        ...
        /**
         * Executes before CommerceML updating order and before cart calculation
         * Allows to add additional data for order update
         *
         * @param array                        $cart       Cart data
         * @param array                        $order_data Order data
         * @param \Tygh\Common\OperationResult $result     Operation result
         */
        fn_set_hook('commerceml_order_storage_update_order_cart_calculation_pre', $cart, $order_data, $result);
         ...
         * @param \Tygh\Common\OperationResult $result     Operation result
         */
        fn_set_hook('commerceml_order_storage_update_order_cart_calculation_post', $cart, $order_data, $result);

        [$order_id, $order_status] = fn_update_order($cart, $order_id);
    ...
    public function getOrderData($id)
    {
        ...
        return fn_get_order_info($order_id, false, true, false, false, CART_LANGUAGE, ['limit_by_company' => true]);
    }
