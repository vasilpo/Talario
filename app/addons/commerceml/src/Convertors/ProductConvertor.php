...
class ProductConvertor
{
    ...
    public function convert(SimpleXmlElement $element, ImportStorage $import_storage, $is_product_creatable = true)
    {
        ...
        if ($element->has('groups')) {
            ...
            if ($groups && $groups->has('id')) {
                foreach ($groups->getAsStringList('id', []) as $item) {
                    $product->categories[] = IdDto::createByExternalId($item);
                }
            }
        }
    ...
    private function convertVariationAttributes(SimpleXmlElement $element, ProductDto $product, ImportStorage $import_storage)
    {
        ...
        foreach ($element->get('product_features/product_feature', []) as $item) {
            if (!$has_combination_id && $item->has('id')) {
                ...
                if ($product_variant->variation_feature_values->has($feature_external_id)) {
                    $selected_features_coll = $product->product_feature_values->get($feature_external_id)->selected_values;
                } else {
                    $selected_features_coll = new ProductFeatureSelectedValueDtoCollection();
                }

                $feature_id = IdDto::createByExternalId($feature_external_id);
                $selected_feature_value = ProductFeatureSelectedValueDto::create($item->getAsString('value'), $feature_variant_external_id_dto);
                $selected_features_coll->add($selected_feature_value);

                $product_variant->variation_feature_values->add(ProductFeatureValueDto::create(
                    $feature_id,
                    $selected_features_coll
                ));
                ...
                if ($product->variation_feature_values->has($feature_external_id)) {
                    $selected_features_coll = $product->product_feature_values->get($feature_external_id)->selected_values;
                } else {
                    $selected_features_coll = new ProductFeatureSelectedValueDtoCollection();
                }
                ...
                if ($product->product_feature_values->has($feature_external_id)) {
                    $selected_values = $product->product_feature_values->get($feature_external_id)->selected_values;
                    foreach ($selected_values->getAll() as $selected_value) {
                        $feature_variant_external_id_dto = $selected_value->value;
    ...
    private function convertPropertiesValues(SimpleXmlElement $element, ProductDto $product, ImportStorage $import_storage)
    {
        ...
        foreach ($element->get('properties_values/property_values', []) as $item) {
            ...
            if ($product->product_feature_values->has($product_feature_external_id->getId())) {
                $selected_features_coll = $product->product_feature_values->get($product_feature_external_id->getId())->selected_values;
            } else {
                $selected_features_coll = new ProductFeatureSelectedValueDtoCollection();
            }

            $product_feature_value = ProductFeatureSelectedValueDto::create($product_feature_value, ProductFeatureConvertor::getVariantExternalId(
                $item->getAsString('id'),
                $product_feature_value,
                $product_feature_value
            ));

            $selected_features_coll->add($product_feature_value);
        ...
        foreach ($element->get('value_fields/value_field', []) as $item) {
            ...
            $property_name = mb_strtolower(trim($item->getAsString('name')));
            $property_value = $item->getAsString('value');
            $matched_properties = [];
            ...
            foreach ($matched_properties as $property_id => $property_data) {
                if ($property_data['type'] === 'float') {
                    $value = (float) str_replace(',', '.', $property_value);
                } elseif ($property_data['type'] === 'yesno') {
                    $value = YesNo::toId(SimpleXmlElement::normalizeBool($property_value));
    ...
    private function convertProductFieldsToProductProperties(SimpleXmlElement $element, ProductDto $product, ImportStorage $import_storage)
    {
        ...
        if ($short_description_source !== 'none') {
            ...
            if ($element->hasAndNotEmpty($short_description_source)) {
                if ($short_description_source === 'description') {
                    $value = nl2br($element->getAsString($short_description_source));
                } else {
    ...
    private function tryConvertPropertyValueToLocalProperty(
        ...
        foreach ($product_feature_value->selected_values->getAll() as $selected_feature_value) {
            if ($selected_feature_value->value_id) {
                ...
                $variant = $import_storage->findEntity(ProductFeatureVariantDto::REPRESENT_ENTITY_TYPE, $selected_feature_value->value_id->getId());
            } else {
                $variant = null;
            }
            ...
            foreach ($matched_properties as $property_id => $property_data) {
                if ($variant && $variant->name) {
                    ...
                    $value = (string) $selected_feature_value->value;
                }
    ...
    private function convertManufacturerToBrandFeatureVariant(
        ...
        $feature_id = IdDto::createByExternalId($this->brand_external_id);
        $selected_values_coll = new ProductFeatureSelectedValueDtoCollection();
        $manufacturer_selected_value = ProductFeatureSelectedValueDto::create($element->getAsString('manufacturer/name'), $manufacturer->getEntityId());
        $selected_values_coll->add($manufacturer_selected_value);

        $manufacturer_value = ProductFeatureValueDto::create(
            $feature_id,
            $selected_values_coll
        );

        $product->product_feature_values->add($manufacturer_value);
