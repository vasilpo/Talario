...
class TestProductStorage extends ProductStorage
{
    ...
    public function findProductFeatures(array $product_feature_ids = [])
    {
        ...
        foreach ($product_feature_ids as $feautre_id) {
    ...
    public function findProductFeatureVariants(int $product_feature_id, $lang_code = null): array
    {
        ...
        if (isset($this->product_features[$product_feature_id][$lang_code]['variants'])) {
            foreach ($this->product_features[$product_feature_id][$lang_code]['variants'] as $variant) {
    ...
    public function findProductFeatureType(int $product_feature_id): string
    {
        ...
        foreach ($this->product_features as $product_feature) {
            if (empty($product_feature[$lang_code]['feature_type'])
                || empty($this->product_features[$product_feature_id][$lang_code]['variants'])
            ) {
                continue;
            }

            $result = $product_feature[$lang_code]['feature_type'];
        }
...
class TestImportRepository extends ImportRepository
{
    ...
    public function findByCatalogId($catalog_id)
    {
        ...
        foreach ($this->storage as $storage) {
            $import_dto = (isset($storage->catalog_id) && $storage->catalog_id === $catalog_id) ? $this->storage : null;
