...
class ProductConvertorXmlTest extends StorageBasedTestCase
{
    ...
    public function dpConvert()
    {
        return [
            ...
            $this->getConvertLocalPropertyCase(),
        ];
    }
...
XML;
    ...
    public function get207ImportCase4()
    {
        ...
        $selected_feature_values_coll7 = new ProductFeatureSelectedValueDtoCollection();
        $selected_feature_value7 = ProductFeatureSelectedValueDto::create(
            'cefcc889-a79d-11e3-9417-002590d8abfd',
            IdDto::createByExternalId('51cd5c98-a79d-11e3-9417-002590d8abfd#cefcc889-a79d-11e3-9417-002590d8abfd')
        );
        $selected_feature_values_coll7->add($selected_feature_value7);

        $product2->product_feature_values->add(ProductFeatureValueDto::create(
            IdDto::createByExternalId('51cd5c98-a79d-11e3-9417-002590d8abfd'),
            $selected_feature_values_coll7
        ));

        $selected_feature_values_coll8 = new ProductFeatureSelectedValueDtoCollection();
        $selected_feature_value8 = ProductFeatureSelectedValueDto::create(
            'Stels',
            IdDto::createByExternalId('brand1c#' . md5('Stels'))
        );
        $selected_feature_values_coll8->add($selected_feature_value8);

        $product2->product_feature_values->add(ProductFeatureValueDto::create(
            IdDto::createByExternalId('brand1c'),
            $selected_feature_values_coll8
        ));
...
XML;
    ...
    public function getConvertLocalPropertyCase()
    {
        $product1 = new ProductDto();
        $product1->is_creatable = true;
        $product1->id = IdDto::createByExternalId('75c55206-01d1-11e8-9381-002590d8abfc');
        $product1->name = TranslatableValueDto::create('Велосипед Формат');
        $product1->categories[] = IdDto::createByExternalId('6e4bef5d-a11d-11e3-9416-002590d8abfd');
        $product1->properties->add(PropertyDto::create('weight', 0.16));
...
<Товар>
    ...
    <ЗначенияРеквизитов>
        <ЗначениеРеквизита>
            <Наименование>НоменклатураВес</Наименование>
            <Значение>0.16</Значение>
        </ЗначениеРеквизита>
    </ЗначенияРеквизитов>
</Товар>
XML;
        return [$xml, [$product1], true, [], ['catalog_convertor.weight_property_source_list' => 'НоменклатураВес']];
    }
}
