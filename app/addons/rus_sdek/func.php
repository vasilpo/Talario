 ...
 * @return void
 */
function fn_rus_sdek_place_suborders_pre($order_id, $cart, $auth, $action, $issuer_id, &$suborder_cart, $key_group, $group)
{
    if (
 ...
 * @return void
 */
function fn_rus_sdek_pre_update_order(array &$cart, $order_id)
{
    ...
    foreach ($cart['product_groups'] as $group_id => $group) {
        ...
        foreach ($group_shipping as $shipping_id => $shipping) {
            if (
                empty($shipping['data'])
                || empty($shipping['module']) || $shipping['module'] !== 'sdek'
