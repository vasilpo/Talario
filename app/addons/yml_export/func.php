...
function fn_yml_addon_install()
{
    ...
    // Adds marketplace mapping settings
    if (fn_allowed_for('MULTIVENDOR')) {
        $company_ids = array_merge($company_ids, (array) fn_get_runtime_company_id());
    }
 ...
 */
function fn_yml_get_offers_features($company_id = 0)
{
    $offers_feature = db_get_array('SELECT param_key, param_data FROM ?:yml_param WHERE param_type = ?s AND company_id = ?i', 'offer', $company_id);
...
/**
 * @param array<string, array<string, string>> $features   Features of product offers
 * @param int                                  $company_id Company ID
 *
 * @return void
 */
function fn_yml_update_offers_features(array $features, $company_id = 0)
{
...
function fn_yml_update_price_list($price_id, $price_list)
{
    ...
    if (!empty($price_id)) {
        ...
        if (fn_allowed_for('ULTIMATE') && (int) $company_id !== 0 && (int) $price['company_id'] !== (int) $company_id) {
            fn_set_notification('E', __('error'), __('access_denied'), '', 'company_access_denied');

            return false;
        }

        if (fn_allowed_for('MULTIVENDOR') || fn_allowed_for('ULTIMATE') && (int) $company_id === 0) {
 ...
 *
 * Actions performed:
 *  - Add marketplace params to the table of product offers for the vendor.
 *
 * @param array<string, string|int|array<string, string>> $company_data Company data
 ...
 */
function fn_yml_export_update_company(array $company_data, $company_id, $lang_code, $action)
{
    if ($action !== 'add') {
        return;
    }

    $fields = ['param_type, param_key, param_data'];

    $marketplace_feature_data = db_get_array('SELECT ' . implode(', ', $fields) . ' FROM ?:yml_param WHERE company_id = ?i', 0);

    if (empty($marketplace_feature_data)) {
        return;
    }

    foreach ($marketplace_feature_data as $offers_param) {
        $data = [
            'param_type' => $offers_param['param_type'],
            'param_key'  => $offers_param['param_key'],
            'param_data' => $offers_param['param_data'],
            'company_id' => $company_id,
        ];

        db_query('INSERT INTO ?:yml_param ?e', $data);
    }
}
