...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($mode == 'update_offers') {
        ...
        if (!empty($_REQUEST['data']['ym_features'])) {
            ...
            if (fn_allowed_for('MULTIVENDOR') && !$company_id && isset($_REQUEST['company_id'])) {
                $company_id = (int) $_REQUEST['company_id'];
                $suffix .= '&company_id=' . $company_id;
            }

            fn_yml_update_offers_features($_REQUEST['data']['ym_features'], $company_id);
        }
    } elseif ($mode == 'update') {
...
if ($mode === 'offers_params') {
    ...
    $offers_features_values = fn_yml_get_offers_features($company_id);
