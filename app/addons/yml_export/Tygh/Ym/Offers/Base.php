...
class Base
{
    ...
    public function build($product)
    {
        ...
        $main_category = isset($product['main_category']) ? $product['main_category'] : $product['category_id'];
        ...
        $offer = array(
            ...
            'items' => array(
                'url' => $this->escapeUrl($product['product_url'] . '&storefront_id=' . $this->options['storefront_id']),
        ...
        if (
            empty($offer['items']['model'])
            && !empty($this->options['yml2_model_categories'][$main_category])
            && !empty($this->options['yml2_model_select'][$main_category]['value'])
            && $this->options['yml2_model_select'][$main_category]['value'] === 'yml2_model'
            && $this->offer_type === 'vendor'
        ) {
            if (strpos($this->options['yml2_model_categories'][$main_category], 'feature') === false) { //exclude empty product feature values
                $offer['items']['model'] = $this->options['yml2_model_categories'][$main_category];
            } else {
                $offer['items']['model'] = null;
            }
        }

        if (empty($offer['items']['typePrefix']) && !empty($this->options['yml2_type_prefix_categories'][$main_category])) {
            if (strpos($this->options['yml2_type_prefix_categories'][$main_category], 'feature') === false) {
                $offer['items']['typePrefix'] = $this->options['yml2_type_prefix_categories'][$main_category];
            } else {
                $offer['items']['typePrefix'] = null;
            }
        }
    ...
    protected function getImageUrl($image_pair)
    {
        ...
        if ($this->options['image_type'] == 'detailed') {
            if (!empty($image_pair['detailed'])) {
                $url = Storage::instance('images')->getUrl($image_pair['detailed']['relative_path'], '', $this->options['storefront_url']);
            }
        } else {
    ...
    protected function buildFeatures($product, &$offer)
    {
        ...
        if ($this->offer_type === 'vendor') {
            if (!empty($product['yml2_type_prefix'])) {
                $offers_feature['param_data']['typePrefix'] = [
                    'type' => 'product',
                    'value' => 'yml2_type_prefix'
                ];
            } elseif (!empty($this->options['yml2_type_prefix_select'][$product['main_category']])) {
                $offers_feature['param_data']['fallback_typePrefix'] = $offers_feature['param_data']['typePrefix'];
                $offers_feature['param_data']['typePrefix'] = $this->options['yml2_type_prefix_select'][$product['main_category']];
            }

            if (!empty($product['yml2_model'])) {
                $offers_feature['param_data']['model'] = [
                    'type' => 'product',
                    'value' => 'yml2_model'
                ];
            } elseif (!empty($this->options['yml2_model_select'][$product['main_category']])) {
                $offers_feature['param_data']['fallback_model'] = $offers_feature['param_data']['model'];
                $offers_feature['param_data']['model'] = $this->options['yml2_model_select'][$product['main_category']];
            }
        }
        ...
        foreach ($offers_feature['param_data'] as $feature_code => $feature_data) {
            if (strpos($feature_code, 'fallback_') !== false) {
                continue;
            }
            ...
            if (!in_array($feature_code, ['model', 'typePrefix'])) {
                continue;
            }

            if (
                (!empty($offer['items'][$feature_code])
                || !isset($offers_feature['param_data']['fallback_' . $feature_code])
                || empty($product[$offers_feature['param_data']['fallback_' . $feature_code]['value']]))
            ) {
                continue;
            }

            if ($feature_code === 'model' && !empty($this->options['yml2_model_categories'][$product['main_category']])) {
                continue;
            }
            if ($feature_code === 'typePrefix' && !empty($this->options['yml2_type_prefix_categories'][$product['main_category']])) {
                continue;
            }
            $offer['items'][$feature_code] = $product[$offers_feature['param_data']['fallback_' . $feature_code]['value']];
        }
    }
    ...
    protected function getFeaturesValues($offer_type)
    {
        ...
        if ($this->offer_type !== 'vendor') {
            unset($offers_feature['common']['param_data']['model'], $offers_feature['common']['param_data']['typePrefix']);
        }
