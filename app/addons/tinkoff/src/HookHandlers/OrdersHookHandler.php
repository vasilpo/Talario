...
class OrdersHookHandler
{
     ...
     *
     * Action performed:
     *    - Allows to manage order status at returning from T-Bank.
     *
     ...
     *
     * Actions performed:
     *     - Creates a data set for the closing receipt for T-Bank
    ...
    public function onChangeOrderStatusPost(
        ...
        if (
            ...
            || !empty($order_info['payment_info']['addons.tinkoff.final_receipt_sent'])
        ) {
            return;
        }
        ...
        if (empty($response['Success'])) {
            ...
            fn_update_order_payment_info(
                ...
                [
                    'addons.tinkoff.final_receipt_sent' => __('no')
                ]
            );
        } else {
            fn_set_notification(NotificationSeverity::NOTICE, __('notice'), __('addons.tinkoff.final_receipt_sent'));
            fn_update_order_payment_info(
                (int) $order_info['order_id'],
                [
                    'addons.tinkoff.final_receipt_sent' => __('yes')
                ]
            );
        }
    }
