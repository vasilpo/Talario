...
function fn_rus_online_cash_register_change_order_status($status_to, $status_from, $order_info)
{
    ...
    if (
        !empty($statuses_paid[$status_to])
        && empty($statuses_paid[$status_from])
        ...
        !empty($statuses_refund[$status_to])
        && empty($statuses_refund[$status_from])
        && (
            $order_data->isStatusPaid()
        ...
        !empty($statuses_prepaid[$status_to])
        && empty($statuses_prepaid[$status_from])
        && !$order_data->isStatusPaid()
        && !$order_data->isStatusPrepaid()
    ) {
