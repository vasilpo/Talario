...
class ReceiptFactoryTest extends ATestCase
{
    ...
    public function setUp(): void
    {
        ...
        \Tygh\Tygh::$app->register(new \Tygh\Addons\RusTaxes\ServiceProvider());
    }
    ...
    public function testCreateReceiptFromOrder(array $order, $currency, $allocate_discount_by_unit, $prices_with_taxes, $total_discount_item_types_filter, $priority_item_type, $expected)
    {
        ...
        $this->assertEqualsWithDelta($expected, $receipt->toArray(), 0.0001);

        if ($currency !== 'RUB') {
            $this->assertEqualsWithDelta($order['total'] * 10, $receipt->getTotal(), 0.0001);
        } else {
            $this->assertEqualsWithDelta($order['total'], $receipt->getTotal(), 0.0001);
        }
    }
