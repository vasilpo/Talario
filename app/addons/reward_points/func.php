    ...
    if (!empty($points_info)) {
        ...
        if ($order_statuses[$status_to]['params']['inventory'] == 'I' && $order_statuses[$status_from]['params']['inventory'] == 'D') {
            if (!empty($points_info['in_use']['points'])) {
                ...
                if (defined('ORDER_MANAGEMENT')) {
                    Registry::set(
                        'reward_points.order_management',
                        [
                            'log_id'   => $log_id,
                            'points'   => $points_info['in_use']['points'],
                            'order_id' => $order_info['order_id'],
                        ]
                    );
                }
        ...
        if ($order_statuses[$status_to]['params']['inventory'] == 'D' && $order_statuses[$status_from]['params']['inventory'] == 'I') {
            if (!empty($points_info['in_use']['points'])) {
                ...
                if ($points_info['in_use']['points'] > fn_get_user_additional_data(POINTS, $order_info['user_id'])) {
                    ...
                    if (defined('ORDER_MANAGEMENT')) {
                        $previous_log = Registry::ifGet('reward_points.order_management', []);

                        if (
                            !empty($previous_log)
                            && $points_info['in_use']['points'] === $previous_log['points']
                            && $order_info['order_id'] === $previous_log['order_id']
                        ) {
                            db_query('DELETE FROM ?:reward_point_changes WHERE change_id IN (?n)', [$log_id, $previous_log['log_id']]);
                        }
                    }
                }
            }
