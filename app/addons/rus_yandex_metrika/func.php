...
function fn_yandex_metrika_oauth_info()
{
    ...
    $view = Tygh::$app['view'];
    $storefront_id = $view->getTemplateVars('selected_storefront_id');

    if ($storefront_id) {
        $params['storefront_id'] = (int) $storefront_id;
    }

    $settings = Settings::instance($params);
    $application_id = $settings->getValue('application_id', 'rus_yandex_metrika');
    $application_password = $settings->getValue('application_password', 'rus_yandex_metrika');
    if (!fn_string_not_empty($application_id) || !fn_string_not_empty($application_password)) {
        $view->assign([
    ...
    return __('yandex_metrika_oauth_info_part2', [
        '[auth_uri]' => 'https://oauth.yandex.ru/authorize?response_type=code&client_id=' . $application_id . '&state=' . $storefront_id,
        '[edit_app_uri]' => 'https://oauth.yandex.ru/client/edit/' . $application_id,
    ]);
}
 ...
 * Common functions
 *
 * @param array<string, int|null> $params Request parameters
 *
 * @return bool
 */
function fn_yandex_metrika_sync_goals(array $params = [])
{
    $settings = Settings::instance($params);

    $counter_number = $settings->getValue('counter_number', 'rus_yandex_metrika');
    ...
    $selected_goals = $settings->getValue('collect_stats_for_goals', 'rus_yandex_metrika');

    $ext_goals = [];
    $res = fn_yandex_metrika_rest_client('get', "/management/v1/counter/{$counter_number}/goals.json", [], [], $params);
    ...
    foreach ($goals_scheme as $goal_name => $goal) {
        ...
        if (!empty($ext_goals[$ext_goal_name])) {
            if (empty($selected_goals[$goal_name]) || $selected_goals[$goal_name] == 'N') {
                fn_yandex_metrika_rest_client(
                    'delete',
                    "/management/v1/counter/{$counter_number}/goal/" . $ext_goals[$ext_goal_name]['id'],
                    [],
                    [],
                    $params
                );
            ...
            if (!empty($selected_goals[$goal_name]) && $selected_goals[$goal_name] == 'Y') {
                ...
                fn_yandex_metrika_rest_client(
                    'post',
                    "/management/v1/counter/{$counter_number}/goals",
                    ['goal' => $goal],
                    [],
                    $params
                );
 ...
 * @param array<string, string>   $data    Data to send within the request
 * @param array<string, string>   $headers Request headers
 * @param array<string, int|null> $params  Request parameters
 *
 * @return array|false API response of false on error
 */
function fn_yandex_metrika_rest_client($type, $url, array $data = [], array $headers = [], array $params = [])
{
    ...
    if (!isset($headers['Authorization'])) {
        if ($oauth_token === null) {
            $oauth_token = Settings::instance($params)->getValue('auth_token', 'rus_yandex_metrika');
        }
