...
/**
 * Adapter between Talario's physical schedule model and the legacy
 * ec_table_booking_system storefront calendar.
 *
 * Talario resources/rules are the business-facing source of schedule data.
 * This service mirrors that data into Ecarter's per-product structure so the
 * existing storefront calendar keeps working without exposing its UI to a vendor.
 */
class BookingBridgeService
{
    /** @var ScheduleResourceService */
    ...
    /**
     * Saves one weekly schedule for a product and mirrors it to Ecarter.
     * First bridge version intentionally supports one slot per weekday.
     */
    ...
    private function ensureResource($product_id)
    {
        ...
        $resource_id = (int) $this->schedule_resources->createResource([
            'name' => $product_name,
            'status' => 'A',
        ]);
    ...
    private function syncEcarter($product_id, $from_date, $to_date, $duration, array $days)
    {
        ...
        $current = db_get_row(
            'SELECT quantity_selector, show_price_date, blocked_date, minimum_booking_time '
