...
$load_owned_product = static function ($product_id) use ($company_id) {
    ...
    $product = db_get_row(
        'SELECT p.product_id, p.company_id, p.status, pd.product '
        . 'FROM ?:products AS p '
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $mode === 'save_schedule') {
    ...
    $schedule_data = isset($_REQUEST['schedule_data']) && is_array($_REQUEST['schedule_data'])
...
if ($mode === 'manage') {
    ...
    try {
        ...
        $locations = array_values(array_filter(
            $schedule_service->getLocations(),
            static function (array $location) {
