...
if ($mode === 'autocomplete_city') {
    ...
    if (defined('AJAX_REQUEST') && $params['q']) {
        ...
        $cities = fn_cities_find_cities($params);
        $select = fn_cities_format_to_autocomplete($cities);
    ...
    if (defined('AJAX_REQUEST')) {
        ...
        if (!empty($params['check_country']) && $params['check_country'] !== 'undefined') {
            ...
            if (!empty($params['check_state']) && $params['check_state'] !== 'undefined') {
                $state_code = db_get_field(
                    'SELECT b.code FROM ?:state_descriptions as a LEFT JOIN ?:states as b ON a.state_id = b.state_id WHERE a.state = ?s',
                    $params['check_state']
                );
        ...
        $data = $cities_service->getCityId($location);
        ...
        Tygh::$app['view']->display('addons/rus_sdek2/views/shippings/components/services/sdek2.tpl');
        exit;
    }
