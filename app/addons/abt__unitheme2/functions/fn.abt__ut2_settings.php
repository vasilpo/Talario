...
$name = $k . '.' . $sub_k;
if (!empty($sub_item['is_size_input'])){
$size_input = &$data[$section][$name];
fn_abt__ut2_settings_validate_size_input($size_input);
}
...
function fn_abt__ut2_settings_validate_size_input(&$input){
if (preg_match('/^\d+px$/', $input)) {
$input = ltrim($input, '0');
return;
}
