        ...
        switch ($options['master_products.exported_products']) {
            ...
            case EximProducts::PRODUCTS_THAT_VENDORS_CAN_SELL:
                if (empty($conditions)) {
                    ...
                    $zero_condition_added = false;
                    $conditions = array_map(static function ($c) use ($exclusive_condition, $zero_condition, &$zero_condition_added) {
                        $res = str_replace($exclusive_condition, $zero_condition, $c);

                        if (!$zero_condition) {
                            $zero_condition_added = $res === $zero_condition;
                        }

                        return $res;
                    }, $conditions);

                    if (!$zero_condition_added) {
                        $conditions[] = $zero_condition;
                    }
                }
                break;
