...
$operation_result = DemoData::getFile([
'file' => 'banners.zip',
'addon' => 'ab__category_banners',
],
true,
Registry::get('config.dir.var') . "ab__data/ab__category_banners/demodata/");
if ($operation_result->isSuccess()) {
$data = $operation_result->getData();
...
$category_id = db_get_field('SELECT category_id FROM ?:categories WHERE level=1 AND status = ?s AND is_trash = ?s ?p', 'A', 'N', fn_get_company_condition('category_id'));
}
if (empty($category_id)) {
...
$image_types = [
'category_banners_main_image' => 'M',
'category_banners_list_image_image' => 'L',
'category_banners_short_list_image_image' => 'S',
];
...
$image_name = preg_replace('/_image$/', '', $var_name);
fn_attach_image_pairs($image_name, 'category_banner', $banner_image_id);
