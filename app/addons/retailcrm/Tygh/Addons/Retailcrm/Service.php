...
class Service
{
    ...
    public function createRetailCrmOrder(array $order)
    {
        ...
        if (!$order_data['status'] || $order['status'] === OrderStatuses::INCOMPLETED) {
            $this->logger->info(
                sprintf('Order #%d skipped, storefront not configured.', $order['order_id']),
                __METHOD__
            );
    ...
    protected function syncOrder($order)
    {
        ...
        if (!isset($order['shipping_ids']) || $order['shipping_ids'] == reset($chosen_shipping_ids)) {
            ...
            foreach ($cart['product_groups'] as &$product_group) {
                $product_group['chosen_shippings'] = [];
                foreach ($shippings as $_shipping) {
                    if ($_shipping['group_name'] !== $product_group['name']) {
                        continue;
                    }
                    $product_group['chosen_shippings'][] = $_shipping;
                }
            }
            unset($_shipping);
            unset($product_group);
