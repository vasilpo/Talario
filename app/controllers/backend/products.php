...
if ($mode === 'manage' || $mode === 'p_subscr') {
    ...
    // Action buttons
    if ($mode === 'manage') {
        // Hide Export found products button, if products are not available
        if (empty($products)) {
            Registry::del('navigation.dynamic.actions.export_found_products');
        }

        // For global search results page
        if (
            empty($_REQUEST['result_ids'])
            || !$_REQUEST['result_ids'] === 'content_manage_products,tools_manage_products_buttons'
        ) {
            Registry::del('navigation.dynamic.actions.clone_selected');
            Registry::del('navigation.dynamic.actions.export_selected');
            Registry::del('navigation.dynamic.actions.delete_selected');
        }
    }
}
