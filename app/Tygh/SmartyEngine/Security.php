...
use Smarty\Resource\StringPlugin;

class Security extends \Smarty\Security
{
    /**
     * @var string[]
     */
    protected $php_disabled_modifiers = [
        ...
        'is_readable', 'is_uploaded_file', 'is_writable', 'is_writeable', 'linkinfo', 'lstat', 'parse_str', 'ini_set',
        ...
        'fn_exim_put_csv', 'fn_copy_by_ftp', 'preg_replace', 'preg_replace_callback', 'spl_autoload_register',
        'register_shutdown_function', 'register_tick_function', 'set_error_handler', 'set_exception_handler',
        'session_set_save_handler', 'sqlite_create_aggregate', 'sqlite_create_function', 'posix_getlogin',
        'posix_ttyname', 'getenv', 'get_current_user', 'proc_get_status', 'get_cfg_var', 'disk_free_space',
        'disk_total_space', 'diskfreespace', 'getcwd', 'getlastmo', 'getmygid', 'getmyinode', 'getmypid', 'getmyuid',
        'extract', 'pathinfo', 'realpath', 'stat', 'gzfile', 'readgzfile', 'getimagesize', 'imagecreatefromgif',
        'imagecreatefromjpeg', 'imagecreatefrompng', 'imagecreatefromwbmp', 'imagecreatefromxbm', 'imagecreatefromxpm',
        'ftp_put', 'ftp_nb_put', 'exif_read_data', 'read_exif_data', 'exif_thumbnail', 'exif_imagetype', 'hash_file',
        'hash_hmac_file', 'hash_update_file', 'md5_file', 'sha1_file', 'highlight_file', 'php_strip_whitespace',
        'get_meta_tags', ''
    ];
    ...
    {
        if (
            empty($compiler->getTemplate())
            || empty($compiler->getTemplate()->getSource())
            || empty($compiler->getTemplate()->getSource()->handler)
            || !$compiler->getTemplate()->getSource()->handler instanceof StringPlugin
        ) {
            return true;
        }
