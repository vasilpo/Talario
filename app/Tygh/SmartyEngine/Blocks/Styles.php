...
use Tygh\Registry;
use Tygh\Storage;
use Tygh\Themes\Styles as ThemeStyles;

class Styles implements BlockHandlerInterface
{
    /**
     * phpcs:ignore
     ...
     * @return mixed|string
     *
     * @throws Tygh\Exceptions\DeveloperException Dev exception.
     */
    //phpcs:ignore
        ...
        if ($styles || $inline_styles) {
            ...
            if (
                ...
                $preloaded_resources = Registry::getOrSetCache(
                    ...
                    [],
                    ['storefront', 'lang'],
                    static function () use ($obj, $file_id_in_storage) {
                        ...
                        return $obj->getPreloadFontResources(
                            $css,
                            CART_LANGUAGE,
            ...
            $content .= '<link type="text/css" rel="stylesheet" ' .
                (empty($params['link_media']) ? '' : 'media="' . $params['link_media'] . '" ') .
                'href="' .
                $filename .
     ...
     * @internal
     */
    public function getPreloadFontResources($css, $language_code, $max_fonts_to_preload, Styles $obj): array
    {
        $font_resources = [];
        ...
        foreach ($css_parse_result['font_faces'] as $font_face_spec) {
            ...
            $font_face_properties = $obj->getFontFaceProperties($properties);
            ...
            foreach ((array) $font_face_properties['src'] as $sources_string) {
                preg_match_all(
                    '~url\(' .
                    '["\']*' .
                    '(?P<urls>(?:http|/|\.).+?)' .
        ...
        $font_resource_priorities = $obj->getFontsFromThemeStyle(Registry::get('runtime.layout'));
     ...
     * @internal
     */
    public function getFontFaceProperties(array $properties)
    {
        ...
        foreach ($properties as $property_string) {
            ...
            if ($property === 'src') {
                $value = array_merge((array) $font_face_properties['src'], (array) $value);
            }
     ...
     * @internal
     */
    public function getFontsFromThemeStyle(array $layout)
    {
        if (!$layout['style_id']) {
            return [];
        }
        ...
        $style = ThemeStyles::factory($layout['theme_name'])->get($layout['style_id'], ['parse' => true]);
        $priority = 0;
