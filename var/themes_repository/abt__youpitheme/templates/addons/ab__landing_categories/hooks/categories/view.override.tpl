{strip}{if !empty($ab__lc_landing_categories)}
    ...
    {/capture}
    {$thumbnail_width = $settings.abt__yt.addons.ab__landing_categories.thumbnail_width|default:$settings.Thumbnails.category_lists_thumbnail_width}
    {$thumbnail_height = $settings.abt__yt.addons.ab__landing_categories.thumbnail_height|default:$settings.Thumbnails.category_lists_thumbnail_height}
    ...
    {$show_max_item=$category_data.ab__lc_subsubcategories|default:0}
                ...
                <div class="ab-lc-landing">
                    <div class="head">
                        <a href="{$item1.href|fn_url}">
                            <div class="image">
                                {if $item1.main_pair || $item1.abt__yt_mwi__icon}

                                        {include file="common/image.tpl"
                                        show_detailed_link=false
                                        images=$item1.main_pair|default:$item1.abt__yt_mwi__icon
        ...
        {hook name="categories:view_description"}
		    {if $category_data.description || $runtime.customization_mode.live_editor}
		        <div class="ab-category-description ty-wysiwyg-content ty-mb-l" {live_edit name="category:description:{$category_data.category_id}"}>{$category_data.description nofilter}</div>
		    {/if}
		{/hook}
{/if}{/strip}