    ...
    <div class="ab-fn-parent ab-fn-block-{$block.grid_id}_{$block.block_id} {$block.user_class} clearfix">
        <div id="ab__fn-first-level-{$block.grid_id}_{$block.block_id}" class="ab-fn-first-level ab-fn-clipped {$properties.ab__fn_display_type} {$template_name} active">
            ...
            {hook name="ab__fast_navigation:first_level_item"}
                <div data-item-id="{$item@key}" data-item-index="{$item@index}" class="ab-fn-first-level-item{if $first_level_scroller} scroller-item{/if} ty-column{$properties.ab__fn_number_of_columns_desktop} {$item.class} ab-fn-dont-allow-link{!$second_level}">
                    {hook name="ab__fast_navigation:first_level_item"}
        ...
        {if $second_level}
            {foreach $items as $item}
                ...
                <div id="ab__fn-second-level-{$elem_id}_{$smarty.const.CART_LANGUAGE}" class="ab-fn-second-level {if $second_level_scroller} ab-fn-second-level-scroller{/if}" data-childs-count="{$item.subitems|count}" data-add-delimeter="{if $second_level_scroller}false{else}true{/if}">
