...
{if $pagination.total_pages > 1}
    ...
    <div class="ty-pagination" id="ut2_pagination_block{if $smarty.capture.pagination_open == "Y"}_bottom{/if}">
        ...
        <a data-ca-scroll=".cm-pagination-container" href="{"`$c_url`&page=1`$extra_url`"|fn_url}" data-ca-page="1" class="cm-history ty-pagination__item ty-pagination__range {if $pagination.current_page == 1}mute{/if} {$ajax_class} {$ajax_full_render_class}" data-ca-target-id="{$id|cat:$extra_id}">1</a>
        {/if}
        ...
        {/if}
        {if $settings.ab__device !== "desktop"}
        <a data-ca-scroll=".cm-pagination-container" href="{"`$c_url`&page=`$pagination.total_pages``$extra_url`"|fn_url}" data-ca-page="{$pagination.total_pages}" class="cm-history ty-pagination__item ty-pagination__range {if $pagination.current_page == $pagination.total_pages}mute{/if} {$ajax_class} {$ajax_full_render_class}" data-ca-target-id="{$id|cat:$extra_id}">{$pagination.total_pages}</a>
        {/if}
