    ...
    <div class="ut2-menu__2nd-col">
        {foreach from=$row item="item2" name="item2"}
            <div class="ut2-menu__2nd-item" data-elem-index="{$second_level_counter}">
                ...
                <div class="ut2-menu__2nd-item__header{if $item2.abt__ut2_mwi__icon} ut2-mwi-icon-wrap{/if}{if empty($item2.$childs)} no-items{/if}">
                    <a {if $settings.ab__device === "desktop" || !$item2.childs}href="{$item2_url|default:"javascript:void(0)"}"
                        ...
                        <div class="ut2-menu__2nd-submenu__wrapper">
                            {if $item1.abt__ut2_mwi__dropdown === "YesNo::YES"|enum}
                                ...
                                {foreach from=array_chunk($item2.$childs, ceil($item2.$childs|count / $cols_tree_level), true) item="item2_childs"}
                                    {if $item2_childs}
                                        <div class="ut2-menu__3rd-list {if $item1.abt__ut2_mwi__dropdown === "YesNo::YES"|enum}{if $item2_childs|count > 19}multicol {/if}{else}{if $item2_childs|count > $Viewlimit}clipped {/if}{/if}" data-cols-count="{$settings_cols}" style="{if $item2_childs|count > $Viewlimit && $item1.abt__ut2_mwi__dropdown ==="YesNo::NO"|enum}--menu-items:{$Viewlimit};{/if}{if $item1.abt__ut2_mwi__dropdown === "YesNo::NO"|enum && $settings.ab__device !== "mobile"}width:{$col_width}%;{if $settings_cols > 2}max-width:{$col_width}%{/if}{/if}">
                                            ...
                                            {if $item2.$name && $item2.href && $settings.ab__device === "mobile"}
                                                <div class="ut2-menu__3rd-item item-3rd-has-more-cat-link">
                                                    <a href="{$item2.href|fn_url}" {if $item2_url && $item2.new_window == "YesNo::YES"|enum} target="_blank"{/if}
                                            ...
                                            {foreach from=$item2_childs item="item3" name="item3"}
                                                ...
                                                <div class="ut2-menu__3rd-item">
                                                    <a href="{$item3_url|default:"javascript:void(0)"}"{if $item3_url && $item3.new_window == "YesNo::YES"|enum} target="_blank"{/if}
                                                       class="ut2-menu__3rd-link {if $item3.class}{$item3.class}{/if}"
                                                       title="{$item3.$name nofilter}">
                            ...
                            {else}
                                {if $item2.$childs}
                                    <div class="ut2-menu__3rd-list {if $item1.abt__ut2_mwi__dropdown === "YesNo::YES"|enum}{if $item2.$childs|count > 19}multicol {/if}{else}{if $item2.$childs|count > $Viewlimit}clipped {/if}{/if}" data-cols-count="{$settings_cols}" {if $item2.$childs|count > $Viewlimit && $item1.abt__ut2_mwi__dropdown === "YesNo::NO"|enum}style="--menu-items:{$Viewlimit};"{/if}>
                                        {hook name="blocks:topmenu_dropdown_3levels_col_elements"}
                                        {if $item2.$name && $item2.href && $settings.ab__device === "mobile"}
                                            <div class="ut2-menu__3rd-item item-3rd-has-more-cat-link">
                                                <a href="{$item2.href|fn_url}" {if $item2_url && $item2.new_window == "YesNo::YES"|enum} target="_blank"{/if}
                                                   ...
                                                   title="">
                                                    ...
                                                    {/if}
                                                    <span class="ut2-menu__3rd-link__text">
                                                        <span class="ut2-menu__3rd-link__name">{__('all')} {$item2.$name nofilter}</span>
                                                    </span>
                                                </a>
                                            </div>
                                        {/if}
                            ...
                            {if $item2.$childs|count > $Viewlimit && $item1.abt__ut2_mwi__dropdown === "YesNo::NO"|enum}
                                <a href="{if $block.properties.abt__ut2_view_more_btn_behavior|default:'view_items' === 'view_items'}javascript:void(0);" onclick="$(this).prev().addClass('view');$(this).addClass('hidden');{else}{$item2_url|default:"javascript:void(0)"}" rel="nofollow{/if}" class="ut2-more-btn" title=""><span>{__("more")}</span></a>
                            {/if}
