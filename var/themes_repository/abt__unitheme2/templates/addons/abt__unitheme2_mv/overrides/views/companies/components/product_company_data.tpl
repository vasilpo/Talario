{* Reset overrides position from Products *}

{if "MULTIVENDOR"|fn_allowed_for && ($company_name || $company_id) && $settings.Vendors.display_vendor == "Y" && !$details_page}
    <div class="ty-control-group paypal-adaptive-vendor-name{if !$capture_options_vs_qty} product-list-field{/if}{if !empty($product.paypal_verification.verified) && $product.paypal_verification.verified == "verified"} paypal-adaptive-vendor-name-text{/if}">
