...
{if $show_features && $product.variation_features_variants}
    {capture name="variation_features_variants"}
        ...
        {foreach $product.variation_features_variants as $variation_feature}
            {if
                $variation_feature.display_on_catalog === "YesNo::YES"|enum
                && ($variation_feature.filter_style != 'color' || ($variation_feature.filter_style == 'color' && $settings.abt__ut2.product_list.product_variations.display_color_separately == 'dont_display' ))
            }
                ...
                {if $limit > 0}
                    ...
                    <div class="ut2-lv__features-description">{$variation_feature.description}</div>
                    ...
                    <div class="ut2-lv__features-item">
                        ...
                        {foreach $variation_feature.variants as $variant}
                            {if $printed_variations < $limit && ($variant.product_id || $addons.product_variations.variations_show_all_possible_feature_variants === "YesNo::YES"|enum)}
                                ...
                                {if $variation_feature.filter_style == 'color'}
                                    {if $settings.abt__ut2.product_list.product_variations.display_as_links[$settings.ab__device] === "YesNo::YES"|enum && $variant.product_id}
                        ...
                        {/foreach}
                        {if $variation_feature.variants|count > $limit}<span class="ut2-lv__more">+{$variation_feature.variants|count - $limit}</span>{/if}
                    </div>
                {/if}
