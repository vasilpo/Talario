...
{if $settings.abt__ut2.product_list.show_rating_num == "YesNo::YES"|enum} ut2-show-rating-num{/if}"
     ...
     data-ca-product-review-reviews-stars-is-half="{$is_half_rating}"
        ...
        {/if}
></div>
{if ($show_rating_num|default:$settings.abt__ut2.product_list.show_rating_num) == "YesNo::YES"|enum}
    <span class="ut2-rating-stars-num">{$accurate_rating}</span>
{/if}
{if $product.reviews_count}<div class="cn-reviews">({__("product_reviews.reviews", [$product.reviews_count])})</div>{/if}
{if $link}
    </a>
{elseif $button}
    </button>
